...
class UITestPantallaInicial {
    ...
    fun lanzarPantallaInicial() {
        val viewModel = mockk<ViewModelPrincipal>(relaxed = true)
    ...
    //Test que comprueba que al haberse cargado los datos, se ejecuta la nevagción
    @Test
    fun `Comprobar_funcionalidad_navegacion`() {
        val viewModel = mockk<ViewModelPrincipal>(relaxed = true)
        val navegar = mockk<() -> Unit>(relaxed = true)

        val estado = MutableStateFlow(ModelPrincipal(isLoading = true, succes = false))
        ...
        estado.value = ModelPrincipal(isLoading = false, succes = true)
